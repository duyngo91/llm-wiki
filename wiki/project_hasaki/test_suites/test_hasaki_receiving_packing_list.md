...
| TC-PL-010 | Validate import — Qty request = 0 (boundary) | R2 | Negative | Boundary Value Analysis | File import có dòng Qty = 0 | 1. Upload file với Qty = 0 | Lỗi "Dòng N – Số lượng yêu cầu không hợp lệ" (phải số dương) | AI-Inferred từ R2 (boundary: Qty phải > 0) | ⏳ |
...
| TC-PL-020 | App — SL thực nhận = 10% threshold (boundary không cảnh báo) | R19 | Positive | Boundary Value Analysis | Packing list Qty = 10; nhập thực nhận = 11 (đúng 10%) | 1. Nhập SL thực nhận = 11 2. Submit | Không hiện cảnh báo; lưu thành công | AI-Inferred từ R19 (cảnh báo khi >10%, không phải ≥10%) | ⏳ |

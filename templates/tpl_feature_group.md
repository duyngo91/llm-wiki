---
aliases: [Tên Feature Group, feature-group-slug]
tags: [qa/feature-group-index, qa/feature-group/{{feature-group}}]
status: Draft
project:
feature_group: {{feature-group}}
created: {{date:YYYY-MM-DD}}
updated: {{date:YYYY-MM-DD}}
---

# 🧩 Feature Group: {{title}}

## Tổng quan
- **Feature group:** `{{feature-group}}`
- **Mục đích:** Gom các Feature Specs và Test Suites cùng một phạm vi nghiệp vụ để AI/con người đọc theo luồng end-to-end.
- **Raw source chính:**
- **Test Plan liên quan:** [[...]]

## Feature Specs Trong Group
| Feature | Vai trò trong group | Status | Gate | Ghi chú |
|:--------|:--------------------|:-------|:-----|:-------|
| [[...]] | | Draft | Gate 1 | |

## Test Suites Trong Group
| Test Suite | Feature cover | Số TC | Blocked coverage | Status |
|:-----------|:--------------|:------|:-----------------|:-------|
| [[...]] | [[...]] | | | Draft |

## API Specs Trong Group
| API Spec | Feature cover | API/Interface cover | Open questions | Status |
|:---------|:--------------|:--------------------|:---------------|:-------|
| [[...]] | [[...]] | API-001 | | Draft |

## Open Questions & Blocked Coverage
| Feature/Test Suite | Question/Blocked item | Owner | Trạng thái | Ghi chú |
|:-------------------|:----------------------|:------|:-----------|:-------|
| [[...]] | | PO/BA/Dev | Open | |

## 📎 Nguồn chưa ingest
*Roll-up các nguồn `⏳ Chưa đọc` / `❓ Không đọc được` từ `## Nguồn tài liệu` của các spec trong group. SSOT ở từng spec — bảng này chỉ tổng hợp để review & bổ sung sau. Regenerate mỗi lần ingest.*

| Nguồn (Figma/Drive/URL/PDF) | Spec liên quan | Trạng thái | Open question | Ghi chú |
|:----------------------------|:---------------|:-----------|:--------------|:-------|
| [tên](url) | [[...]] | ⏳ / ❓ | Q-0xx | |

## Impact & Regression Notes
| Change ID / Source | Feature(s) ảnh hưởng | Regression candidate | Trạng thái |
|:-------------------|:---------------------|:---------------------|:-----------|
| | | | |

## 📅 Changelog
| Thời gian | Version | Nội dung thay đổi | Nguồn |
|:----------|:--------|:------------------|:------|
| {{date:YYYY-MM-DD HH:mm:ss}} | v1.0 | Khởi tạo Feature Group | |

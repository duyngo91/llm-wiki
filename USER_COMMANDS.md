...
## 🔍 Tra cứu task Hasaki (mặc định không lưu vault)
...
Lưu ý:
- Nếu dùng biến thể `và tải hình về`, ảnh sẽ được lưu vào `raw_sources/project_hasaki/assets` (có ghi vào vault).
- Nếu muốn fetch + phân tích + lưu task markdown + cập nhật kanban, dùng `import task HSK-XXXXX` (`/import-hasaki-task`).

...
KANBAN_PATH = VAULT_DIR / "KANBAN.md"
...
def sync_completed_task(test_suite_link):
    ...
    suite_path = VAULT_DIR / f"{suite_clean}.md"
    ...
    if feature_link_match:
        ...
        feat_path = VAULT_DIR / f"{feat_clean}.md"
        if feat_path.exists():
...
def run_sync():
    ...
    synced_suites = [os.path.basename(suite) for suite in suite_links if sync_completed_task(suite)]
    if synced_suites:
...
def run_daily_sync(project, date_str):
    ...
    daily_note_path = VAULT_DIR / daily_note_rel
    ...
    if KANBAN_PATH.exists():
        kanban_lines = KANBAN_PATH.read_text(encoding="utf-8").split("\n")
...
def run_verify():
    ...
    for info in all_pages.values():
        ...
        for link in link_pattern.findall(content):
            ...
            possible_paths = [VAULT_DIR / normalized_link, VAULT_DIR / f"{normalized_link}.md"]
...
def main():
    if len(sys.argv) < 2:
        ...
        print("For daily-sync: python .agents/skills/wiki-sync-helper/scripts/wiki_sync.py daily-sync --project <project_name> --date <YYYY-MM-DD>")
        return 1
    ...
    if command == "daily-sync":
        ...
        if not project or not date_str:
            ...
            return 1
        return run_daily_sync(project, date_str)
